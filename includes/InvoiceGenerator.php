...
class InvoiceGenerator {
    ...
    public function createFromOrder($orderId) {
        ...
        // Parse order details - handle both old and new formats
        $orderData = null;
        $products = [];
        
        // Try order_details first (old format)
        ...
        // Fallback to products_json (new format)
        if (empty($products) && !empty($order['products_json'])) {
        ...
        if (!empty($products)) {
            foreach ($products as $item) {
                ...
                $unitPrice = floatval($item['final_price'] ?? $item['price'] ?? 0);
                $vatRate = 0.00; // Non-VAT payer (neplátce DPH)
                
                $totalItemWithoutVat = $quantity * $unitPrice;
                $vatAmount = 0; // No VAT
                $totalItemWithVat = $totalItemWithoutVat; // Same as without VAT
        ...
        // Add shipping if present (column name is shipping_cost in database)
        $shippingPrice = floatval($order['shipping_cost'] ?? $order['shipping_price'] ?? 0);
        if ($shippingPrice > 0) {
            $shippingVat = 0; // No VAT for non-VAT payer
