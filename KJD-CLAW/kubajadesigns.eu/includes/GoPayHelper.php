...
class GoPayHelper {
    ...
    public function createPayment($orderData) {
        ...
        if ($httpCode < 200 || $httpCode >= 300) {
            $errorMsg = "Failed to create GoPay payment (HTTP $httpCode)";
            if ($curlError) {
        ...
        if (!isset($data['id']) || !isset($data['gw_url'])) {
            throw new Exception('Invalid GoPay response - missing id or gw_url: ' . $response);
        }
